...
def compilar():
    ...
    subprocess.run(["nvcc", "-O2", "-o", "../cuda/karatsuba_cuda.exe",
                    "../cuda/karatsuba_cuda.cu"], check=True)
...
def benchmark():
    ...
    print(f"{'Tamanho':>10} | " + " | ".join(f"{v:>12}" for v in VERSOES))
    print("=" * (12 + len(VERSOES) * 15))
    for n in TAMANHOS:
        linha = f"{n:>10,} |"
        for nome, exe in VERSOES.items():
            try:
                tempos = [medir(exe, n) for _ in range(REPETICOES)]
